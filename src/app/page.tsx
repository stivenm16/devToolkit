...
function Home() {
  ...
  useEffect(() => {
    const parsedUser = searchParams.get("user")
      ? tryToParseJson(searchParams.get("user")!)
      : localStorage.getItem("user")
      ? tryToParseJson(localStorage.getItem("user")!)
      : null;
  ...
  return (
    <Layout>
      <div className='flex justify-center  items-center min-h-96 pt-36 flex-col z-10'>
        <div className='text-center animate-fade-in'>
          <h1 className='text-6xl font-bold antialised text-white'>
            {(loggedUser &&
              `Welcome ${(loggedUser as any).user?.displayName}`) ||
      ...
      <div className='waves waves-opacity absolute bottom-0 left-0 w-full overflow-hidden leading-none'>
        <svg
          data-name='Layer 1'
          xmlns='http://www.w3.org/2000/svg'
          viewBox='0 0 1200 120'
          preserveAspectRatio='none'
          className='relative block h-[273px]'
        >
          <path
